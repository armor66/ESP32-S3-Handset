...
/*DSView time chart in conjunction with RxEN(which starts pattern)
 and TxEN(to make sure that adc conversion occurs while transmitting):*/
    ...
    uint8_t counter4 = 0;   //counter0, counter1, counter3, counter4, counter5;
    // counter0 = counter1 = counter3 = counter4 = counter5 = 0;
...
//  uint32_t raw_slider;        //raw_sw_b, raw_sw_c;
    ...
    for (uint16_t i = 0; i < numBytesRead; i+=ADC_RESULT_BYTE)
    {
        ...
        switch(pData->type2.channel)
        {
            ...
            case ADC1_CHANNEL_4:
                ...
                ++counter4;  //it may drop by 1, that is why
                break;
            // case ADC1_CHANNEL_5:
            //     adc_throttle += pData->type2.data;
            //     break;
            // case ADC1_CHANNEL_6:
                ...
                // ++counter5;  //it may drop by 1, that is why
            //     adc_yaw += pData->type2.data;
                // break;
            default:
                adcNoData = true;
    ...
    raw_pitch = adc_pitch/SAMPLES;    //(adc_pitchSum - adc_pitchMax - adc_pitchMin)/2;  
    raw_throttle = adc_throttle/SAMPLES;
    raw_yaw = adc_yaw/counter4;
