...
static void displayIdleScreen(bool init)
{
    ...
    static int16_t last_voltage = 0x00;     //my@
    ...
    static uint8_t last_modelMatch_VL = 0xFF;   //my@
    static uint8_t last_modelMatch_ST = 0xFF;   //my@

    int16_t voltage = (int16_t)handset_vbat;    //my@
    ...
    if (changed == 0)
    {
        ...
        changed |= last_voltage != voltage ? CHANGED_TEMP : 0;      //my@
        ...
        changed |= last_modelMatch_VL != config.GetModelMatch_VL() ? CHANGED_MODELMATCH : 0;    //my@
        changed |= last_modelMatch_ST != config.GetModelMatch_ST() ? CHANGED_MODELMATCH : 0;    //my@
    }

    if (changed)
    {
        last_message = disp_message;
        last_voltage = voltage;     //my@
        ...
        last_modelMatch_VL = config.GetModelMatch_VL(); //my@
        last_modelMatch_ST = config.GetModelMatch_ST(); //my@
...
//my@
    case STATE_CALIBR_ENDPOINTS:
        values_min = 0;
        values_max = display->getValueCount((menu_item_t)state_machine.getParentState())-1;
        values_index = config.GetCalEnd_points();
        break;
    case STATE_CALIBR_CENTRVALS:
        values_min = 0;
        values_max = display->getValueCount((menu_item_t)state_machine.getParentState())-1;
        values_index = config.GetCalCenrt_vals();
        break;
    case STATE_REVERSE_GIMBALS:
        values_min = 0;
        values_max = display->getValueCount((menu_item_t)state_machine.getParentState())-1;
        values_index = config.GetReverse_gmbls();
        break;
    case STATE_MODEL_NUMBER:
        values_min = 0;
        values_max = display->getValueCount((menu_item_t)state_machine.getParentState())-1;
        values_index = config.GetModelMatch_VL();
        break;
    case STATE_MODEL_STATUS:
        values_min = 0;
        values_max = display->getValueCount((menu_item_t)state_machine.getParentState())-1;
        values_index = config.GetModelMatch_ST();
        break;
    }
}
...
{
//my@
    if((state_machine.getParentState() == STATE_CALIBR_ENDPOINTS) ||
        (state_machine.getParentState() == STATE_CALIBR_CENTRVALS))
    {
...
//my@
    if(state_machine.getParentState() == STATE_CALIBR_ENDPOINTS) endPointsCalibration();
    if(state_machine.getParentState() == STATE_CALIBR_CENTRVALS) centrValsCalibration();
}
...
//my@
    if((state_machine.getParentState() == STATE_CALIBR_ENDPOINTS) ||
        (state_machine.getParentState() == STATE_CALIBR_CENTRVALS)) { return; }
//@
    uint8_t values_count = values_max - values_min + 1;
    values_index = (values_index - values_min + 1) % values_count + values_min;
...
//my@
        case STATE_CALIBR_ENDPOINTS:
            config.SetCalEnd_points(values_index);
            break;
        case STATE_CALIBR_CENTRVALS:
            config.SetCalCenrt_vals(values_index);
            break;
        case STATE_REVERSE_GIMBALS:
            config.SetReverse_gmbls(values_index);
            break;

        case STATE_MODEL_NUMBER:
            config.SetModelMatch_VL(values_index);
            break;
        case STATE_MODEL_STATUS:
            config.SetModelMatch_ST(values_index);
            break;
//@
        default:
            break;
...
//my@
// Telemetry
static void displayTelemetry(bool init)
{
    display->displayTelemetry();
}

// Gimbals
static void displayGimbals(bool init)
{
    display->displayGimbals();
}
//-------------------------------------------------------------------
...
//my@
//Calibration fsm
fsm_state_entry_t const calibration_menu_fsm[] = {
    {STATE_CALIBR_ENDPOINTS, nullptr, displayMenuScreen, 20000, value_menu_events, ARRAY_SIZE(value_menu_events)},
    {STATE_CALIBR_CENTRVALS, nullptr, displayMenuScreen, 20000, value_menu_events, ARRAY_SIZE(value_menu_events)},
    {STATE_REVERSE_GIMBALS, nullptr, displayMenuScreen, 20000, value_menu_events, ARRAY_SIZE(value_menu_events)},
    {STATE_LAST}
};

//Model Match fsm
fsm_state_entry_t const modelMatch_menu_fsm[] = {
    {STATE_MODEL_NUMBER, nullptr, displayMenuScreen, 20000, value_menu_events, ARRAY_SIZE(value_menu_events)},
    {STATE_MODEL_STATUS, nullptr, displayMenuScreen, 20000, value_menu_events, ARRAY_SIZE(value_menu_events)},
    {STATE_LAST}
};
//@
// Main menu FSM
...
fsm_state_event_t const calibration_menu_events[] = {MENU_EVENTS(calibration_menu_fsm)};    //my@
fsm_state_event_t const modelMatch_menu_events[] = {MENU_EVENTS(modelMatch_menu_fsm)};      //my@

fsm_state_entry_t const main_menu_fsm[] = {
    {STATE_MODELMATCH, nullptr, displayMenuScreen, 20000, modelMatch_menu_events, ARRAY_SIZE(modelMatch_menu_events)},  //my@
    ...
    {STATE_CALIBRATION, nullptr, displayMenuScreen, 20000, calibration_menu_events, ARRAY_SIZE(calibration_menu_events)},   //my@
    {STATE_LAST}
};
...
fsm_state_event_t const linkstats_confirm_events[] = {
    ...
    {EVENT_UP, PUSH(gimbals_menu_fsm)},         //{EVENT_UP, ACTION_POPALL},
    {EVENT_DOWN, PUSH(telemetry_menu_fsm)},     //{EVENT_DOWN, ACTION_POPALL}
    {EVENT_LEFT, ACTION_POPALL}
};
...
//my@
// Telemetry FSM
fsm_state_event_t const telemetry_confirm_events[] = {
    {EVENT_TIMEOUT, GOTO(STATE_TELEM)},
    {EVENT_LONG_ENTER, PUSH(main_menu_fsm)},
    {EVENT_LONG_RIGHT, PUSH(main_menu_fsm)},
    {EVENT_UP, PUSH(linkstats_menu_fsm)},       //{EVENT_UP, ACTION_POPALL},
    {EVENT_DOWN, PUSH(gimbals_menu_fsm)},        //{EVENT_DOWN, ACTION_POPALL}
    {EVENT_LEFT, ACTION_POPALL}
};
fsm_state_entry_t const telemetry_menu_fsm[] = {
    {STATE_TELEM, nullptr, displayTelemetry, 1000, telemetry_confirm_events, ARRAY_SIZE(telemetry_confirm_events)},
    {STATE_LAST}
};

// Gimbals FSM
fsm_state_event_t const gimbals_confirm_events[] = {
    {EVENT_TIMEOUT, GOTO(STATE_TELEM)},
    {EVENT_LONG_ENTER, PUSH(main_menu_fsm)},
    {EVENT_LONG_RIGHT, PUSH(main_menu_fsm)},
    {EVENT_UP, PUSH(telemetry_menu_fsm)},       //{EVENT_UP, ACTION_POPALL},
    {EVENT_DOWN, PUSH(linkstats_menu_fsm)},      //{EVENT_DOWN, ACTION_POPALL}
    {EVENT_LEFT, ACTION_POPALL}
};
fsm_state_entry_t const gimbals_menu_fsm[] = {
    {STATE_GIMBALS, nullptr, displayGimbals, 1000, gimbals_confirm_events, ARRAY_SIZE(gimbals_confirm_events)},
    {STATE_LAST}
};
//@
// Entry FSM
...
fsm_state_event_t const idle_events[] = {
    ...
    {EVENT_LONG_ENTER, PUSH(main_menu_fsm)},
    {EVENT_LONG_RIGHT, PUSH(main_menu_fsm)},
    {EVENT_UP, PUSH(telemetry_menu_fsm)},       //m@
