...
void modelMatchToMsp()
{
    mspPacket_t packet;
    packet.reset();
    packet.makeCommand();                       // 1
    packet.function = MSP_SET_RX_CONFIG;        //0x2D
    packet.addByte(MSP_ELRS_MODEL_ID);          //0x0A
    // packet.addByte(config.GetModelMatch_VL());
    ...
    crsfRouter.AddMspMessage(&packet, CRSF_ADDRESS_BROADCAST, CRSF_ADDRESS_CRSF_RECEIVER);
    // CRSF::AddMspMessage(&packet, CRSF_ADDRESS_CRSF_RECEIVER);

    packet.reset();
    // if (!handset->IsArmed()) // Do not send while armed
    // {
