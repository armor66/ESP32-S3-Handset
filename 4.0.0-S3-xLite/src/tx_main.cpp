...
static uint32_t LinkStatsLastReported_Ms = 0;  //uart@
...
//my@ variables
uint8_t airBat_Val = 0;
bool isRXconnected = false;
bool mspTransferActive = false;   //moved from "loop"
char flightMode[8] = {0};
int32_t latitude;
int32_t longitude;
int16_t speed;
uint8_t sats;
bool modelChanged = false;        //extern in common.h
uint32_t TLMpacketReported = 0;
uint8_t *p_tlmBuff = CRSFinBuffer;
...
void ICACHE_RAM_ATTR GenerateSyncPacketData(OTA_Sync_s * const syncPtr)
{
  ...
  {
//my@ syncPtr->UID5 ^= (~crsfTransmitter.modelId) & MODELMATCH_MASK;
    syncPtr->UID5 ^= (~config.GetModelMatch_VL()) & MODELMATCH_MASK;
  }
}
...
void SetRFLinkRate(uint8_t index) // Set speed of RF link
{
  ...
  isRXconnected = false;    //my@ telemRx
...
static void UpdateConnectDisconnectStatus()
{
    ...
    (connectionState == awaitingModelId && (now - rfModeLastChangedMS) > ExpressLRS_currAirRate_RFperfParams->DisconnectTimeoutMs))
  {
    setConnectionState(disconnected);
    linkStats.uplink_Link_quality = 0;
    LinkStatsLastReported_Ms = 0; // Notify immediately //uart@
...
//my@ #elif defined(PLATFORM_ESP8266)
//   Stream *serialPort;
...
static void checkSendLinkStatsToHandset(uint32_t now) //uart@
{
  if ((now - LinkStatsLastReported_Ms) > firmwareOptions.tlm_report_interval)
  {
//toDSView    gpio_set_level(GPIO_NUM_45, 1);
...
//ToHandset   crsfRouter.deliverMessage(&otaConnector, (crsf_header_t *)linkStatisticsFrame);
    sendCRSFTelemetryToBackpack(linkStatisticsFrame);   //MSP packet of type `MSP_ELRS_BACKPACK_CRSF_TLM`
    LinkStatsLastReported_Ms = now;
//toDSView    gpio_set_level(GPIO_NUM_45, 0);
  }
}
...
void setup()
{
  ...
  UARTconnected(); //my@new just pretend to execute it
//  BackpackTelemReadyToSend = true;  //bp@ send once (devBackpack.cpp)
}
...
void loop()
{
  ...
  // Update UI devices
  // devicesUpdate(now);
/*my@new to prevent abrupt halt when wifiUpdate or move it after >MODE_STATES clause*/
  if(connectionState != wifiUpdate) devicesUpdate(now);

  // Not a device because it must be run on the loop core
  // checkBackpackUpdate(); firmware update has been requested
  ...
  checkSendLinkStatsToHandset(now); //uart@

  if(isRXconnected)  //after ever telemetry packet received
  {
    isRXconnected = false;
...
// gpio_set_level(GPIO_NUM_45, 1);
      ...
      // {
        ...
        sendCRSFTelemetryToBackpack(CRSFinBuffer);    //MSP packet of type `MSP_ELRS_BACKPACK_CRSF_TLM`
      // }
      if (CRSFinBuffer[0] == CRSF_SYNC_BYTE)
      {
        if(CRSFinBuffer[2] == CRSF_FRAMETYPE_BATTERY_SENSOR)
        {
          airBat_Val = CRSFinBuffer[4];
        }
        else if(CRSFinBuffer[2] == CRSF_FRAMETYPE_FLIGHT_MODE)
        {
          for(uint8_t i = 0; i < (CRSFinBuffer[1] - 2); ++i)
          {
            flightMode[i] = CRSFinBuffer[i + 3];
          }
        }
        else if(CRSFinBuffer[2] == CRSF_FRAMETYPE_GPS)
        {
          latitude = 0;
          latitude = ((int32_t)CRSFinBuffer[3] << 24);
          latitude |= ((int32_t)CRSFinBuffer[4] << 16);
          latitude |= ((int32_t)CRSFinBuffer[5] << 8);
          latitude |= ((int32_t)CRSFinBuffer[6]);

          longitude = 0;
          longitude = ((int32_t)CRSFinBuffer[7] << 24);
          longitude |= ((int32_t)CRSFinBuffer[8] << 16);
          longitude |= ((int32_t)CRSFinBuffer[9] << 8);
          longitude |= ((int32_t)CRSFinBuffer[10]);

          speed = (((int16_t)CRSFinBuffer[11] << 8) + (int16_t)CRSFinBuffer[12]);
          //int16_t ground course 13, 14; uint16_t GPS altitude 15, 16
          sats = CRSFinBuffer[17];
        }
      }
...
//      gpio_set_level(GPIO_NUM_45, 0);
  ...
  if ((now - TLMpacketReported) > 3000)   //RX_CONNECTION_LOST_TIMEOUT)
  {
    airBat_Val = 0;
    memset(flightMode, 0, 8);
    latitude = 0;
    longitude = 0;
    speed = 0;
    sats = 0;
  ...
  {
    //my@ model match state changed locally in config.cpp/TxConfig::SetModelMatch_ST/VL
    if(modelChanged) 
    {
      modelChanged = false;
      modelMatchToMsp();    //defined in lib/VTX/devVTX. file
    }
