...
bool BackpackTelemReadyToSend = true;  //bp@ send once, then false;
...
void sendCRSFTelemetryToBackpack(const uint8_t *data)   //BACKPACK_TELEM_MODE_BLUETOOTH (3)
{
...
static void sendConfigToBackpack()
{
    ...
    packet.addByte(MSP_ELRS_BACKPACK_CONFIG_TLM_MODE); // Backpack tlm mode (0x31)
