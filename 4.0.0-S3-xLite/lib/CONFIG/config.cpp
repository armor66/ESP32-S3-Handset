...
#define ALL_CHANGED         (EVENT_CONFIG_MODEL_CHANGED | EVENT_CONFIG_VTX_CHANGED | EVENT_CONFIG_MAIN_CHANGED | EVENT_CONFIG_FAN_CHANGED\
     | EVENT_CONFIG_MOTION_CHANGED | EVENT_CONFIG_BUTTON_CHANGED | EVENT_CONFIG_VERSION_CHANGED | EVENT_CONFIG_ENDPOINTS_CHANGED | EVENT_CONFIG_CENTRVALS_CHANGED)
...
//@
    ...
    if (version >= 7) {
        ...
        // if (nvs_get_u8(handle, "backpackdisable", &value8) == ESP_OK)    //bp@
            m_config.backpackDisable = 0;                                   //bp@ (enable) value8;
        // if (nvs_get_u8(handle, "backpacktlmen", &value8) == ESP_OK)      //bp@
            m_config.backpackTlmMode = 3;                                   //bp@ (FUTURE) bluetooth value8;
    }
...
//my@
    if (m_modified & EVENT_CONFIG_ENDPOINTS_CHANGED)
    {
    ...
    if (m_modified & EVENT_CONFIG_CENTRVALS_CHANGED)
    {
...
void TxConfig::SetCalEnd_points(uint8_t calEnd_points)
{
    if(GetCalEnd_points() != calEnd_points)
    {
        ...
        m_modified |= EVENT_CONFIG_ENDPOINTS_CHANGED;
    }
}
void TxConfig::SetCalCenrt_vals(uint8_t calCentr_vals)
{
    if(GetCalCenrt_vals() != calCentr_vals)
    {
        ...
        m_modified |= EVENT_CONFIG_CENTRVALS_CHANGED;
    }
}
...
void TxConfig::SetModelMatch_ST(bool modelMatch_ST)
{
    if (GetModelMatch_ST() != modelMatch_ST)
    {
        ...
        m_modified |= EVENT_CONFIG_CENTRVALS_CHANGED | EVENT_CONFIG_MODEL_CHANGED;
    }
}
//@
void
