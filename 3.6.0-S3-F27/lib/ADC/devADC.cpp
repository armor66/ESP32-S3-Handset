...
static uint16_t adc1_chan_mask = BIT(0) | BIT(1) | BIT(3) | BIT(4);     //  | BIT(5) | BIT(6)
static uint16_t adc2_chan_mask = 0;
static adc1_channel_t channel[CHANNELS] = {ADC1_CHANNEL_0, ADC1_CHANNEL_1, ADC1_CHANNEL_3, ADC1_CHANNEL_4}; //, ADC1_CHANNEL_5, ADC1_CHANNEL_6
...
// Initialize continous DMA mode ADC1 Channels 3, 4, 5, 6
bool InitializeContinousADC()
{
  ...
  adc_digi_configuration_t dig_cfg =
  {
      ...
      .sample_freq_hz = 20000,  //20000->890uS //28672, //SOC_ADC_SAMPLE_FREQ_THRES_HIGH, //INT16_MAX
...
/*DSView time chart in conjunction with RxEN(which starts pattern)
 and TxEN(to make sure that adc conversion occurs while transmitting):*/
    ...
    uint32_t raw_sw_b, raw_sw_c;
    //theese vars solely to separate raw_* values and get proper calibration
    uint32_t adc_roll, adc_pitch, adc_throttle, adc_yaw;
    raw_sw_b = raw_sw_c = adc_roll = adc_pitch = adc_throttle = adc_yaw = 0;
    ...
    for (uint16_t i = 0; i < numBytesRead; i+=ADC_RESULT_BYTE)
    {
        ...
        {
            case ADC1_CHANNEL_0:
                // raw_sw_b += pData->type2.data;
                adc_throttle += pData->type2.data;
                break;
            case ADC1_CHANNEL_1:
                // raw_sw_c += pData->type2.data;
            ...
            case ADC1_CHANNEL_4:
                ...
                // adc_pitchMax = max(adc_pitchMax, adc_pitch);
                // adc_pitchMin = min(adc_pitchMin, adc_pitch);
    ...
    raw_sw_b = raw_sw_b/SAMPLES;
    raw_sw_c = raw_sw_c/SAMPLES;
...
/*right button*/
//    gpio_get_level(GPIO_NUM_7)? ChannelData[8] = 172: ChannelData[8] = 1812;
/*left button*/
    gpio_get_level(GPIO_NUM_6)? ChannelData[9] = 172: ChannelData[9] = 1812;

    /*adc 3-pos switches / sliders*/
    // if(raw_sw_b < 1024) {ChannelData[8] = 191;}
    // else if(raw_sw_b > 3072) {ChannelData[8] = 1793;}
    // else {ChannelData[8] = 992;}
    // if(raw_sw_c < 1024) {ChannelData[9] = 191;}
    // else if(raw_sw_c > 3072) {ChannelData[9] = 1793;}
    // else {ChannelData[9] = 992;}

    gpio_get_level(GPIO_NUM_47)? ChannelData[11] = 172: ChannelData[11] = 1812; //power button

/*fake values on telemetry screen*/
    adcCounter = counter;
    adcRaw1 = raw_roll;
    adcRaw2 = raw_pitch;
    adcRaw3 = raw_throttle; //raw_sw_b;
    adcRaw4 = raw_yaw;      //raw_sw_b;
...
//     if (GPIO_PIN_JOYSTICK != UNDEF_PIN) analogReadings[ADC_JOYSTICK] = analogRead(GPIO_PIN_JOYSTICK);
// #endif
// #if defined(GPIO_PIN_PA_PDET)
//     if (GPIO_PIN_PA_PDET != UNDEF_PIN) analogReadings[ADC_PA_PDET] = analogReadMilliVolts(GPIO_PIN_PA_PDET);
// #endif

    if(adcStatus) adcBytes = getChannelValues();
