...
static void displayIdleScreen(bool init)
{
    ...
    static int16_t last_voltage = 0x00;
    ...
    static uint8_t last_modelMatch_VL = 0xFF;
    static uint8_t last_modelMatch_ST = 0xFF;

    int16_t voltage = (int16_t)handset_vbat;
    ...
    if (changed == 0)
    {
        ...
        changed |= last_modelMatch_VL != config.GetModelMatch_VL() ? CHANGED_MODELMATCH : 0;
        changed |= last_modelMatch_ST != config.GetModelMatch_ST() ? CHANGED_MODELMATCH : 0;
    }
    ...
    if (changed)
    {
        ...
        last_modelMatch_VL = config.GetModelMatch_VL();
        last_modelMatch_ST = config.GetModelMatch_ST();
...
fsm_state_event_t const calibration_menu_events[] = {MENU_EVENTS(calibration_menu_fsm)};
fsm_state_event_t const modelMatch_menu_events[] = {MENU_EVENTS(modelMatch_menu_fsm)};

fsm_state_entry_t const main_menu_fsm[] = {
    {STATE_MODELMATCH, nullptr, displayMenuScreen, 20000, modelMatch_menu_events, ARRAY_SIZE(modelMatch_menu_events)},
    ...
    {STATE_CALIBRATION, nullptr, displayMenuScreen, 20000, calibration_menu_events, ARRAY_SIZE(calibration_menu_events)},
    // {STATE_MODELMATCH, nullptr, displayMenuScreen, 20000, modelMatch_menu_events, ARRAY_SIZE(modelMatch_menu_events)},
    {STATE_LAST}
};
...
fsm_state_event_t const idle_events[] = {
    ...
    {EVENT_LONG_ENTER, PUSH(main_menu_fsm)},
    {EVENT_LONG_RIGHT, PUSH(main_menu_fsm)},
    // {EVENT_UP, PUSH(gimbals_menu_fsm)},
    // {EVENT_DOWN, PUSH(linkstats_menu_fsm)},
    {EVENT_UP, PUSH(telemetry_menu_fsm)},
    {EVENT_DOWN, PUSH(linkstats_menu_fsm)}
};
