// #if defined(USE_OLED_SPI) || defined(USE_OLED_SPI_SMALL) || defined(USE_OLED_I2C) || defined(HAS_TFT_SCREEN)my@
...
// my@#ifdef HAS_FIVE_WAY_BUTTON
    if(1)   //my@ restrict long press only by excluding (!handset->IsArmed())
    {
        int key;
        bool isLongPressed;
        ...
        //my@ restrict long press only by adding (!handset->IsArmed()) &&
        if ((!handset->IsArmed()) && (fsm_event != EVENT_TIMEOUT) && isLongPressed)
        {
