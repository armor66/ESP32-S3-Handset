...
#define ENDPOINTS_CHANGED   bit(7)
#define CENTRVALS_CHANGED   bit(0)   //and model match changed
#define ALL_CHANGED         (MODEL_CHANGED | VTX_CHANGED | MAIN_CHANGED | FAN_CHANGED | MOTION_CHANGED | BUTTON_CHANGED | ENDPOINTS_CHANGED | CENTRVALS_CHANGED)
...
void TxConfig::Load()
{
    ...
    if (version < 5 || version > TX_CONFIG_VERSION)
    {
        ...
        // vTaskDelay(pdMS_TO_TICKS(200));  //200mS pause if here
        SetDefaults(true);
        return;
    }
    // gpio_set_level(GPIO_NUM_48, 0);
    // vTaskDelay(pdMS_TO_TICKS(100));      //100mS pause if here
    ...
    if (nvs_get_u32(handle, "roll_M", &value) == ESP_OK)
    {
        m_config.roll_Min = value >> 16;
        m_config.roll_Max = value;
    }
    if (nvs_get_u32(handle, "pitchM", &value) == ESP_OK)
    {
        m_config.pitchMin = value >> 16;
        m_config.pitchMax = value;
    }
    if (nvs_get_u32(handle, "thrtlM", &value) == ESP_OK)
    {
        m_config.thrtlMin = value >> 16;
        m_config.thrtlMax = value;
    }
    if (nvs_get_u32(handle, "yaw__M", &value) == ESP_OK)
    {
        m_config.yaw__Min = value >> 16;
        m_config.calEnd_points = value >> 12 & 0xF;
        m_config.yaw__Max = value & 0xFFF;
    }
    if (nvs_get_u32(handle, "roll_pitch", &value) == ESP_OK)
    {
        m_config.roll_Ctr = value >> 16;
        m_config.pitchCtr = value;
    }
    if (nvs_get_u32(handle, "yawCtr", &value) == ESP_OK)
    {
        m_config.yaw__Ctr = value >> 16;

        m_config.calCentr_vals = value >> 12 & 0xF;
        m_config.reverse_gmbls = value >> 8 & 0xF;

        m_config.modelMatch_ST = value >> 6 & 0x1;
        m_config.modelMatch_VL = value & 0x3F;        //LSB?
    }
    ...
    calibrated = true;  //get true only here
}
...
// #if defined(PLATFORM_ESP32)
    ...
    if (m_modified & ENDPOINTS_CHANGED)
    {
        uint32_t value0 = m_config.roll_Min << 16 | m_config.roll_Max;
        nvs_set_u32(handle, "roll_M", value0);

        uint32_t value1 = m_config.pitchMin << 16 | m_config.pitchMax;
        nvs_set_u32(handle, "pitchM", value1);

        uint32_t value2 = m_config.thrtlMin << 16 | m_config.thrtlMax;
        nvs_set_u32(handle, "thrtlM", value2);

        uint32_t value3 = m_config.yaw__Min << 16 |
        m_config.calEnd_points << 12 | (m_config.yaw__Max & 0xFFF);
        nvs_set_u32(handle, "yaw__M", value3);

        TxConfig::Load();
    }

    if (m_modified & CENTRVALS_CHANGED)
    {
        uint32_t value0 = m_config.roll_Ctr << 16 | m_config.pitchCtr;
        nvs_set_u32(handle, "roll_pitch", value0);

        uint32_t value1 = m_config.yaw__Ctr << 16 |
        m_config.calCentr_vals << 12 |
        m_config.reverse_gmbls << 8 |
        m_config.modelMatch_ST << 6 |
        (m_config.modelMatch_VL & 0x3F);
        nvs_set_u32(handle, "yawCtr", value1);
...
void TxConfig::SetCalEnd_points(uint8_t calEnd_points)
{
    if(GetCalEnd_points() != calEnd_points)
    {
        m_config.calEnd_points = calEnd_points;
        m_config.roll_Min = rollMin_tmp;
        m_config.roll_Max = rollMax_tmp;
        m_config.pitchMin = pitchMin_tmp;
        m_config.pitchMax = pitchMax_tmp;
        m_config.thrtlMin = thrtlMin_tmp;
        m_config.thrtlMax = thrtlMax_tmp;
        m_config.yaw__Min = yawMin_tmp;
        m_config.yaw__Max = yawMax_tmp;
        m_modified |= ENDPOINTS_CHANGED;
    }
}
void TxConfig::SetCalCenrt_vals(uint8_t calCentr_vals)
{
    if(GetCalCenrt_vals() != calCentr_vals)
    {
        m_config.calCentr_vals = calCentr_vals;
        m_config.roll_Ctr = rollCtr_tmp;
        m_config.pitchCtr = pitchCtr_tmp;
        m_config.yaw__Ctr = yawCtr_tmp;
        m_modified |= CENTRVALS_CHANGED;
    }
}
void TxConfig::SetReverse_gmbls(uint8_t reverse_gmbls)
{
    if(GetReverse_gmbls() != reverse_gmbls)
    {
        m_config.reverse_gmbls = reverse_gmbls;
        m_modified |= CENTRVALS_CHANGED;
    }
}
void TxConfig::SetModelMatch_VL(uint8_t modelMatch_VL)
{
    if (GetModelMatch_VL() != modelMatch_VL)
    {
        m_config.modelMatch_VL = modelMatch_VL;
        m_modified |= CENTRVALS_CHANGED;
    }
}
void TxConfig::SetModelMatch_ST(bool modelMatch_ST)
{
    if (GetModelMatch_ST() != modelMatch_ST)
    {
        m_model->modelMatch = modelMatch_ST;
        // m_modified |= MODEL_CHANGED;
        modelChanged = true;
        m_config.modelMatch_ST = modelMatch_ST;
        m_modified |= CENTRVALS_CHANGED | MODEL_CHANGED;
    }
}
...
uint32_t TxConfig::GetCalibratedValue(uint8_t itemIndex)
{
    uint32_t ret;
    switch(itemIndex)
    {
        case ROLL_MIN:
            ret = m_config.roll_Min;
            break;
        case ROLL_CTR:
            ret = m_config.roll_Ctr;
            break;
        case ROLL_MAX:
            ret = m_config.roll_Max;
            break;

        case PITCH_MIN:
            ret = m_config.pitchMin;
            break;
        case PITCH_CTR:
            ret = m_config.pitchCtr;
            break;
        case PITCH_MAX:
            ret = m_config.pitchMax;
            break;

        case THRTL_MIN:
            ret = m_config.thrtlMin;
            break;
        case THRTL_MAX:
            ret = m_config.thrtlMax;
            break;

        case YAW_MIN:
            ret = m_config.yaw__Min;
            break;
        case YAW_CTR:
            ret = m_config.yaw__Ctr;
            break;
        case YAW_MAX:
            ret = m_config.yaw__Max;
            break;
        //GetReverse_gmbls() in config.h
        // case REV_IND:
        //     ret = m_config.reverse_gmbls;
        //     break;
...
// #if defined(PLATFORM_ESP32)      // ESP32 nvs needs to commit every model
        if (commit)
        {
            // gpio_set_level(GPIO_NUM_48, 1);  //end of 200mS pause and start 64 records
            m_modified |= MODEL_CHANGED;
            Commit();
        }
// #endif   // STM32/ESP8266 just needs one commit(removed)
    }
    // vTaskDelay(pdMS_TO_TICKS(20));      //end of 64 records
    // gpio_set_level(GPIO_NUM_48, 0);     //or just 20mS impuls length
