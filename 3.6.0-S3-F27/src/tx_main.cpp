...
device_affinity_t ui_devices[] = {
  {&Handset_device, 1},
  {&AnalogVbat_device, 1}, // 0 = alternate core or 1 = loop core
#ifdef HAS_LED
  {&LED_device, 0},
#endif
#ifdef HAS_RGB
...
//my@new static void HandleUARTout()
// {
//   if (firmwareOptions.is_airport)
//   {
...
//my@new static void HandleUARTin()
// {
...
void loop()
{
  ...
  //my@new HandleUARTout(); // Only used for non-CRSF output
...
/*my@new to prevent abrupt halt when wifiUpdate or move it after >MODE_STATES clause*/
  ...
  if(isRXconnected)  //after ever telemetry packet received
  {
    ...
    // handset->sendTelemetryToTX(linkStatisticsFrame);     //to handset
    // sendCRSFTelemetryToBackpack(linkStatisticsFrame);    //to uart, just for test
   isRXconnected = false;
  ...
  if (TelemetryReceiver.HasFinishedData())
  {
    // if (CRSFinBuffer[0] == CRSF_ADDRESS_USB)
      ...
      // handset->sendTelemetryToTX(CRSFinBuffer);      //to handset
      // sendCRSFTelemetryToBackpack(CRSFinBuffer);     //to uart, merely to confirm
      // }
      if (CRSFinBuffer[0] == 0xC8)
      {
        if(CRSFinBuffer[2] == 0x08)
        {
          airBat_Val = CRSFinBuffer[4];
        }
        else if(CRSFinBuffer[2] == 0x21)
        {
  ...
  // static bool mspTransferActive = false;    //guess should move out of "loop"
  ...
  else if (!MspSender.IsActive())
  {
    ...
    {
      //model match state changed locally in config.cpp/TxConfig::SetModelMatch_ST
      if(modelChanged) 
      {
        modelChanged = false;
      ...
      if (mspData != nullptr)
      {
        sendCRSFTelemetryToBackpack(mspData);   //to uart, merely to confirm
